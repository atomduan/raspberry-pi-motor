...
class KeyTracker:
    ...
    def get_key_event(self):
        ...
        if select.select([sys.stdin], [], [], 0)[0]:
            ...
            elif ch.lower() in ['h', 'j', 'k', 'l', 'b']:  # 添加 'b'
                self.last_key_time = time.time()
                self.combo_hits += 1
...
if __name__ == "__main__":
    ...
    try:
        print("控制说明:")
        print("方向键: 移动小车 | WASD: 控制舵机 | B: 按住鸣铃 | C: 舵机回中 | Q: 退出")
        ...
        while True:
            ...
            # 闹铃控制
                ...
                bell_on()
                tracker.print_info('鸣铃中')

            # 停止检测（包括所有动作和闹铃）
            elif event and event.endswith('_release'):
                stop()
                if tracker.key_states['bell_ring']:
                    bell_off()  # 确保闹铃关闭
